...
function SuggestionBox() {
  ...
  const fetchList = async () => {
    try {
      const tryUrl = `${API_BASE}/api/feedback?role=tpo`;
      let res = null;
      try {
      ...
      if (!res) res = await fetch(`/api/feedback?role=tpo`);
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const body = { fromRole: 'tpo', toRole: 'admin', fromEmail, message };
  ...
  const reply = async (id: string, replyMsg: string) => {
    ...
    try {
      await fetch(`${API_BASE}/api/feedback/${id}/reply`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fromRole: 'tpo', fromEmail, message: replyMsg }) });
      fetchList();
  ...
  return (
    <div className="space-y-4">
      ...
      <Textarea placeholder="Write a suggestion or feedback to Admin" value={message} onChange={(e:any)=>setMessage(e.target.value)} />
      <div className="flex gap-2">
        <Button onClick={send} disabled={loading}>{loading ? 'Sending...' : 'Send to Admin'}</Button>
...
const TPOCommunications = () => {
  return (
    <TPODashboardLayout title="Communications">
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Communications (TPO & Admin)</CardTitle>
          </CardHeader>
          <CardContent>
            <SuggestionBox />
          </CardContent>
...
export default TPOCommunications;
